	...
	public void testExtractFunction() {
		inTransaction(
				session -> {
					...
					session.createQuery("select extract(timezone hour from e.theTime) from EntityOfBasics e")
							.list();
					session.createQuery("select extract(timezone hour minute from e.theTime) from EntityOfBasics e")
							.list();

					session.createQuery("select extract(timezone offset from e.theTimestamp) from EntityOfBasics e")
							.list();
